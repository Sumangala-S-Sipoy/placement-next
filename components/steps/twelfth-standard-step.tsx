...
import { TwelfthStandardDetails } from "@/types/profile"
...
const twelfthStandardSchema = z.object({
...
type TwelfthStandardForm = z.infer<typeof twelfthStandardSchema>

interface TwelfthStandardStepProps {
  data: Partial<TwelfthStandardDetails>
  onNext: (data: Partial<TwelfthStandardDetails>) => Promise<void>
  onPrevious?: () => void
  isLoading: boolean
}

export function TwelfthStandardStep({ data, onNext, onPrevious, isLoading }: TwelfthStandardStepProps) {
  ...
  } = useForm<TwelfthStandardForm>({
    resolver: zodResolver(twelfthStandardSchema),
    defaultValues: {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const uploadResponse = await uploadFile(file, "twelfthMarksCard", {
  ...
  const onSubmit = async (formData: TwelfthStandardForm) => {
    await onNext({
  ...
  // Helper functions
  const hasError = (fieldName: keyof TwelfthStandardForm) => !!errors[fieldName]
  const isFieldTouched = (fieldName: keyof TwelfthStandardForm) => !!touchedFields[fieldName]
  const isFieldValid = (fieldName: keyof TwelfthStandardForm) => isFieldTouched(fieldName) && !hasError(fieldName)
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Details about your 12th standard school/college and location
            </CardDescription>
          </CardHeader>
        ...
        <div className="pt-6 border-t space-y-4">
          <div className="text-sm text-muted-foreground text-center">
            Step 5 of 7 - 12th Standard Details
          </div>
